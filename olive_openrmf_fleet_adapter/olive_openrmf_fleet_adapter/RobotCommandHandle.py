...
class RobotCommandHandle(adpt.RobotCommandHandle):
    ...
    def update_state(self):
        ...
        self.node.get_logger().debug(
           f"update_state: pos=[{self.position[0]:.2f}, "
           f"{self.position[1]:.2f}, {self.position[2]:.2f}], "
           f"on_waypoint={self.on_waypoint}, on_lane={self.on_lane}, "
           f"dock_waypoint_index={self.dock_waypoint_index}, "
           f"target_graph_index="
           f"{self.target_waypoint.graph_index if self.target_waypoint else None}"
        )
        ...
        with self._lock:
            if (self.on_waypoint is not None):  # if robot is on a waypoint
                self.node.get_logger().debug(
                   f"Calling update_current_waypoint("
                   f"waypoint={self.on_waypoint}, yaw={self.position[2]:.2f})"
                )   
