...
from .coordinate_transform import estimate_transform
from .coordinate_transform import estimate_transform_error
...
def initialize_fleet(config_yaml, nav_graph_path, node, use_sim_time, server_uri):
    ...
    transforms = {
        'rmf_to_robot': estimate_transform(
            rmf_coordinates, robot_coordinates),
        'robot_to_rmf': estimate_transform(
            robot_coordinates, rmf_coordinates)}
    transforms['orientation_offset'] = \
        transforms['rmf_to_robot'].get_rotation()
    mse = estimate_transform_error(transforms['rmf_to_robot'],
                                   rmf_coordinates,
                                   robot_coordinates)
    node.get_logger().debug(f"Coordinate transformation error: {mse}")
    node.get_logger().debug(
        "RMF to Robot transform: "
        f"rotation={transforms['rmf_to_robot'].get_rotation()}, "
        f"scale={transforms['rmf_to_robot'].get_scale()}, "
        f"translation={transforms['rmf_to_robot'].get_translation()}")
    node.get_logger().debug(
        "Robot to RMF transform: "
        f"rotation={transforms['robot_to_rmf'].get_rotation()}, "
        f"scale={transforms['robot_to_rmf'].get_scale()}, "
        f"translation={transforms['robot_to_rmf'].get_translation()}")
